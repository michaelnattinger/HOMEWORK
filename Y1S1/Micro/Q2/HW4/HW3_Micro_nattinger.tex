...
\subsection{Part B}
\begin{align*}
u_i(b_i,v_j|v_i) &= (p(v_i - b_i) + q(v_i - b(v_j)))P(b_i>b(v_j))\\
&+ (1-p-q)(v_i - b_i)P(b_i<b(v_j)) + (1/2)(v_i - b_i)P(b_i=b(v_j))\\
\Rightarrow U_i(b_i|v_i) &= \int_0^1 u_i(b_i,v_j|v_i)p(v_j)dv_j\\
&=  \int_0^1(p(v_i - b_i) + q(v_i - b(v_j)))1\{b_i>b(v_j)\}\\
&+ (1-p-q)(v_i - b_i)1\{b_i<b(v_j)\} + (1/2)(v_i - b_i)1\{b_i=b(v_j)\}dv_j\\
&=\int_{0}^{b^{-1}(b_i)}p(v_i - b_i) + q(v_i - b(v_j))dv_j +(1-p-q)(v_i - b_i)(1-b^{-1}(b_i))\\
&= b^{-1}(b_i)p(v_i - b_i) + \int_{0}^{b^{-1}(b_i)}q(v_i - b(v_j))dv_j + (1-p-q)(v_i - b_i)(1-b^{-1}(b_i))
\end{align*}
\subsection{Part C}
...
& (1-p-q)(1-b^{-1}(b_i)) + (1-p-q)(v_i - b_i)\frac{1}{b'(b^{-1}(b_i))} + b^{-1}(b_i)p\\&= \frac{1}{b'(b^{-1}(b_i))}p(v_i - b_i) + q(v_i - b(b^{-1}(b_i)))\frac{1}{b'(b^{-1}(b_i))}
\end{align*}
\begin{align*}
\Rightarrow b'(v_i) (1-p-q)(1-v_i) + (1-p-q) (v_i - b(v_i)) + b'(v_i) v_i p &= p(v_i - b(v_i)) + q(v_i - b(v_i))
\end{align*}
Now, assume $b$ is affine: $b(x) = ax+c$.
\begin{align*}
\Rightarrow a(1-p-q)(1-v_i) + (1-p-q) (v_i - av_i - c) +av_i p &= p(v_i - av_i - c) + q(v_i - av_i - c)
...
\begin{align*}
v_i = 1 \Rightarrow (1-p-q)(1-a-c) + ap &= (p+q)(1-a-c) \\
%\Rightarrow (1-p-q) &= ap + q\\
%\Rightarrow a &= \frac{1-p-2q}{p}\\
v_i = 0 \Rightarrow  (1-p-q)(a - c) &= -(p + q)c\\
\Rightarrow c&= \frac{p+q-1}{4p+3q-2}\\
\Rightarrow a&= \frac{2p + 2q -1}{4p + 3q -2}
...
\begin{align}
b(v) &= \left( \frac{2p + 2q -1}{4p + 3q -2}\right)v +  \frac{p+q-1}{4p+3q-2} \label{eqn:bne}
\end{align}
\subsection{Part D}
...
\item As $q\rightarrow \frac{1}{2}$ (and $p\rightarrow0$), the intercept goes to $1$ and the slope goes to $0$.
\end{itemize}
\section{Question 2}
\subsection{Part A}
...
\subsection{Part C}
Depending on the values of $\beta_1,\beta_2,$ no correlated equilibria can occur as $B,B$ is the only nash equilibrium. %correlated equilibria can be achieved for   $\{(A,A),(B,A),(A,B),(B,B)\}$.
\subsection{Part D}
