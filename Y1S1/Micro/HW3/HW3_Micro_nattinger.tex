...
&= g(q^{'},t) - g(q,t)
\end{align*}
So $g$ has increasing differences in $q,-\tau$. Now, let $q,q^{'}$ be optimal at $t = -\tau,t^{'} = -\tau^{'}$ with $t<t^{'}$. Then, if $q>q^{'}$,
\begin{equation*}
...
The firm's new objective function is $h(x,t) = (1-\tau)P(q)q - c(q)$, where $t=(-\tau)$. $\frac{\partial h}{\partial t} = P(q)q$. If price is decreasing in $q$, which is not unreasonable for this scenario, then $h$ does not necessarily have increasing differences in $q,-\tau$.
