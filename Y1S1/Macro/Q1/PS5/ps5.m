...
% Edits made for pset 5 by Michael B. Nattinger.
mkdir('pings')
% Clear the memory
clear; close all; clc
...
while q<nq && (abs(K1-K0)>tolk || abs(L1-L0)>tollab)
    ...
    % Prices
    r0 = (alpha)*(L0/K0)^(1-alpha) - delta; % <------------------- FIRM PI MAX
    w0 = (1-alpha)*(K0/L0)^(alpha);% <------------------- FIRM PI MAX

    % Pension benefit
    b =  (tau)*L0*w0/(sum(mass(JR:J)));% <------------------- GOVT BC
    ...
    for j=J-1:-1:tW+1 % age
        for ik0=1:nk        % assets today
            ...
            while ik1<nk  	% assets tomorrow
                ...
                cons = (1+r0)*kap0+b-kap1; % <------------------- BC(old)

                if cons<=0
                    util=neg;
                else
                    util = (cons^(1-sigma))/(1-sigma); % <------------------- Utility func (old)
                end

                % Right-hand side of Bellman equation
                v0 = util + beta*v(ik1,j+1); % <------------------- Current period value function
    ...
    for j=tW:-1:1           % age
        for ik0=1:nk          % assets today
            ...
            while ik1 < nk  	% assets tomorrow
                ...
                lab = gamma - (1-gamma).*((1+r0)*kap0-kap1)/((1-tau)*w0*e(j)); % <------------------- Labor supply from question 1
                ...
                % Instantaneous utility
                cons = (1+r0)*kap0+(1-tau)*w0*e(j)*lab - kap1; % <------------------- BC (young)

                if cons<=0
                    util = neg;
                else
                    util = (((cons^gamma)*(1-lab)^(1-gamma))^(1-sigma))/(1-sigma); % <------------------- Utility (young)
                end

                % Right-hand side of Bellman equation
                v0 = util + beta*v(ik1,j+1); % <------------------- Value (young
    ...
    for j=1:J               % iterations over cohort
        ...
        % update k = kp
        if kap(ik1)==maxkap
            warning(['maxcap is binding! j=' num2str(j) ', young'])
        end
        ik0 = ik1;
    end
...
if tau>1e-5 &&q==1 %do these plots for first iter of ss version
%% Plots
...
load('ss.mat','v','mass','ikgen','J','res1');
...
load('no_ss.mat','v','mass','ikgen','J','res2');
...
% [K0, L0, w0, r0, b]
NoSS = [res2, V1_noss, W_noss ]';
SS = [res1, V1_ss , W_ss ]';
T=table(NoSS,SS);
T.Properties.RowNames = {'kap','lab','w','r','b','V1','W'};
table2latex(T)