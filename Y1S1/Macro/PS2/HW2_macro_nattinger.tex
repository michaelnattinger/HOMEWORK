...
&\text{s.t. }  p_t c_{t}^{1,t} + M_{t+1}^{1,t} \leq p_t w_1 \\
& \text{and } p_{t+1} c_{t+1}^{1,t} \leq M_{t+1}^{1,t}.
\end{align*}
The next type of consumer solves the following:\footnote{Note that here I write $M_{t+1}^{2,t}$ on the right hand side to maintain that it will be positive, making the market clearing in the monetary market work as written.}
\begin{align*}
&\max_{c_t^{2,t},c_{t+1}^{2,t},M_{t+1}^{2,t}} \text{ ln } c_{t}^{2,t} + \text{ ln } c_{t+1}^{2,t}\\
&\text{s.t. }  p_t c_{t}^{2,t}  \leq M_{t+1}^{2,t} \\
& \text{and } p_{t+1} c_{t+1}^{2,t} + M_{t+1}^{2,t}\leq p_{t+1}w_2.
\end{align*}
...
&\text{s.t. }  p_1 c_{1}^{2,0} \leq p_1 w_2 + M_{1}^{2,0} .
\end{align*}
