...
c_{t+1} &= \beta c_t(1 - \delta + \alpha z (zk_{t}^{\alpha} + (1-\delta) k_t - c_t)^{\alpha - 1} ).
\end{align*}
...
In this question we consider a model quite similar to the 2-period OG model we studied in class, but with a few differences as described in the problem set.

SPP: The social planner maximizes the utilities of the agents given the resource constraint:
\begin{align*}
&\max_{c_t^{t},c_{t+1}^{t}} N \text{ ln } c_{t}^{t} + N\text{ ln } c_{t+1}^{t}\\
&\text{s.t. } N  c_{t}^{t} + N  c_{t+1}^{t} \leq \frac{N}{2}w_1 + \frac{N}{2} w_2
\end{align*}

CP: Each consumer maximizes their own utility over the two periods, subject to their income constraints.
\begin{align*}
&\max_{c_t^{t},c_{t+1}^{t}} N \text{ ln } c_{t}^{t} + N\text{ ln } c_{t}^{t}\\
&\text{s.t. } N  c_{t}^{t} + N  c_{t}^{t} \leq \frac{N}{2}w_1 + \frac{N}{2} w_2
\end{align*}
