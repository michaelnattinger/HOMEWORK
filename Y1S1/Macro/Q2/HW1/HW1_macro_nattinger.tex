...
Recall that, due to properties of argmax, our choice of $c_t$ is equivalent to the choice of $c_t$ from a different value function, $\tilde{V}(A) = \max_{A'} log(A'/R- A) + \beta \tilde{V}(A')$. We will guess and verify that this implies that we will save a constant portion of our assets.

Let $c_t = aA_t$ for some $a \in (0,1)$. Then, we can combine this fact with $(\ref{eqn:euler})$ to form a guess for our value function: $\tilde{V}(A) = \sum_{i=0}^n \beta^i log((\beta R)^iaA)$. We can plug this into our formulation for the value function:
...
Therefore, the optimal policy function is to consume $1-\beta$ of your assets and to save $\beta$ of your assets.
