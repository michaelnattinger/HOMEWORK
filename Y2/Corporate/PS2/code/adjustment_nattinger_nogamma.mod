...
parameters 
	theta r delta rho sigma gamma psi0;
...
sigma = 0.01;   // std(eps)
gamma = 0.0;    // CRRA utility parameter
psi0 = 0.01;
...
model;
    ...
    q = 1 + psi0*( i - delta * k(-1) )/k(-1);

    // 2. FOC wrt k_{t+1} (equation (5))

    q = sdf(+1) * (theta * z(+1) * k^(theta-1) + q(+1) * (1-delta) + psi0*( (i(+1) - delta * k)^2/(2* k^2) + delta*(i(+1) - delta* k)/k  ) );
	...
	c = z * k(-1)^theta - i - psi0* ( i - delta*k(-1) )^2/(2*k(-1)) ;
