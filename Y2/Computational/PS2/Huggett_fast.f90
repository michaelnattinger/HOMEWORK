...
! calc new q_grid
q_guess = q_min ! we don't have to check q_min bc we know it was not selected last iteration, so we skip
del_sample = ( q_max - q_min)/(n_samples - 1)
do i_sample = 1,(n_samples -2) ! skip first and last point bc we checked them last time and they were suboptimal guesses
if (i_mid ==1 .and. i_sample == n_mid - 1 ) then
q_guess = q_guess + del_sample
q_grid(i_sample) = apr_mid
else 
tot_its = tot_its+1
q_guess = q_guess + del_sample
converged = 0
...
q_grid(i_sample) = apr_sum
end if
end if
end do
i_min = minloc(abs(q_grid))
apr_mid = minval(abs(q_grid))
i_mid = 1
! move to steps around i_min
q_max = q_min + del_sample*(i_min(1) + 1)
q_min = q_min + del_sample*(i_min(1) - 1)
write(*,*) "********************************************"
write(*,*) "a sum =  ", abs(q_grid(i_min))
write(*,*) "best q  = ", q_min+del_sample
write(*,*) "********************************************"
end do
return
...
do i_a = 1,n_a
     write(2,200) pf_a(i_a), pf_a_b(i_a), pmf(i_a,1), pmf(i_a,2), pf_v(i_a), pf_v_b(i_a)!, pf_i_apr(i_a,1), pf_i_apr(i_a,2)!grid_a(i_a), pf_c(i_a), pf_a(i_a), pf_v(i_a), pmf(i_a,1), pmf(i_a,2), pmf_init_flat(i_a,1), pmf_init_flat(i_a,2)!, pf_i_apr(i_a,1), pf_i_apr(i_a,2)
end do
