...
double precision, parameter :: 				cZb 				= 0.2d0	! Depreciation Rate
...
integer						 				  :: 				i_k, i_kpr, i_z,i_start									! Ieration Counters for k_today and k_tomorrow grid
...
double precision 						:: 				budg_g(n_k) ! Allocate Space for Value Function
double precision 						:: 				budg_b(n_k) ! Allocate Space for Value Function
...
do i_k = 1,n_k
	...
	pf_k_b(i_k) 		 	= 0d0
	pf_v_b(i_k)    	     = 0d0
	budg_g(i_k) = cZg*(grid_k(i_k))**(cTHETA)
	budg_b(i_k) = cZb*(grid_k(i_k))**(cTHETA)
end do
...
do while (converged == 0 .and. it < max_it)
	...
	do i_k = 1,n_k ! Loop over all possible capital states
		...
		!end if
		!y_today = ZZ*k_today**(cTHETA)
		if (i_z < 1.5) then
			y_today = budg_g(i_k)
		else
			y_today = budg_b(i_k)
		end if
		do i_kpr = i_start,n_k ! Loop over all possible capital chocies
			...
			if (c_today_temp > 0) then
			!if (i_z < 1.5) then
			!	v_tomorrow = cPgg*pf_v(i_kpr) + cPgb*pf_v_b(i_kpr)
			!else
			!	v_tomorrow = cPbg*pf_v(i_kpr) + cPbb*pf_v_b(i_kpr) !pf_v(i_kpr)
			!end if
			v_tomorrow = Pg*pf_v(i_kpr) + Pb*pf_v_b(i_kpr)
	  ...
	  pf_c_up_b(i_k) = c_today
	  pf_k_up_b(i_k) = k_tomorrow
	...
	diff_k_b  = maxval(abs(pf_k_b - pf_k_up_b))
	diff_v_b  = maxval(abs(pf_v_b - pf_v_up_b))

	!max_diff = diff_c + diff_k + diff_v + diff_c_b + diff_k_b + diff_v_b 
	max_diff = max(diff_c,diff_k,diff_v,diff_c_b,diff_v_b)
